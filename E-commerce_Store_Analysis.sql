...
from customers as c
join customer_behavior as cb on c.CustomerID = cb.CustomerID
join customer_churn as cc on c.CustomerID = cc.CustomerID;
...
-- Churn Flag Label
-- ========================================================= --
select *, case when Churn=1 then 'Churned'
else 'Retained' end as ChurnLabel from churn_analysis;
...
select round(churned.total_churn/total.total_customers * 100,2) as churn_percentage from
(select count(*) as total_churn from churn_analysis where churn=1) as churned,
(select count(*) as total_customers from churn_analysis)as total;
-- Nearly 16.72 percentage of customers have churned.
...
-- With an annual churn rate of 16.72%, the business falls within this normal range, indicating that churn is not significantly high.
